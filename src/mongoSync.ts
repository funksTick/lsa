...
import type { LocalServicesLead, LsaLeadDocument } from './types';
 ...
 */
export async function syncLeadsToMongo(
  mongo: MongoClient,
  dbName: string,
  leads: LocalServicesLead[]
): Promise<LocalServicesLead[]> {
  const col: Collection<LsaLeadDocument> = mongo.db(dbName).collection('lsa_leads');
  const changed: LocalServicesLead[] = [];
  ...
  for (const lead of leads) {
    ...
    if (isChanged) changed.push(lead);
  }
